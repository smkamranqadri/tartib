...
- Task: none in flight. **Slice 23 (the UI language) is built and proved in the browser, and is
  ...
  row to Home and turns the spaces grid into a list with bars. Themes came back on top of it: seven, all dark, bronze the default, generated by
  `frontend/tools/themes.py` and recorded in `kis/intent/themes.md`. A later pass gave every control one height (32px pointer, 44px phone), made the decision card use
