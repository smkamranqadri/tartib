...
`ai_calls(id, created_at, kind, model, capture_id, ok, failure, reason, resets_at, duration_ms, + six token counts)` (migration 0016) is one row per AI call. `capture_id` is null for an Ask and is **not** a foreign key on purpose: deleting a capture must not erase the record that the work was paid for. Rows rather than counters, because rows can answer what a prompt change cost and a counter cannot.
...
**A capture costs about 15,600 input tokens, and only ~2,400 of that is Tartib's prompt.** The other ~13,100 is the CLI's own instructions, sent on every call. This dwarfs anything the app's own prompt does, and it is why an eval run is ~810k input tokens rather than the ~155k the prompt sizes suggest.
