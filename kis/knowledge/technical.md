...
**Why an item waits (slices 28 and 30).** `items.wait_reason` is a short code -- `no_space`, `low_confidence`, `duplicate`, `asked`, `split`, `whole`, `linked`, `relink` -- not display text, because the client already has the matched item and writes the sentence itself. `items.duplicate_of` is recorded **whether or not parking is on**: `TARTIB_DUPLICATE_PARK` (default off) decides only whether a suspected duplicate is held back, so the verdict can be judged against real captures first. Nothing is ever merged; the matched item is untouched.
...
**Links the classifier proposes (slice 33 phase C).** `TARTIB_LINK_PROPOSALS` (default off) adds a `related` field to the proposal schema (`OUTPUT_SCHEMA_LINKS`; a strict schema must require every property, hence a second schema) and one bullet to the prompt through the `{related}` slot; **off, prompt and schema are byte-identical to before**, asserted by test. `related` names at most two of the `i1`..`i8` candidates already shown for duplicates, resolved like `duplicate_of` (`_resolve_related`: invented labels, the duplicate itself and repeats dropped). An item that would have filed waits instead with reason `linked`, unless its space's policy is `file`; otherwise it keeps its own reason and still carries the proposal. `proposal_json` holds `related` only when non-empty. `/api/attention` adds `related: [{id, title, space}]` (`store.related_info`); approve takes `links` (only the listed, proposed ids are kept; absent keeps none, because only the card shows the links -- the item page's "File it" never did, and its approvals used to keep them all unseen; changed after the pre-deploy review, 2026-09-23) and appends `Related: [[A]], [[B]]` by link title (`store.related_line`), after which links behave as any other. `ai_calls.links_on` (migration 0021) records the switch per call, so data from before and after it can be told apart.
...
`python -m tartib.suggest_links --space NAME [--limit 20] [--dry-run]` (slice 34) asks the classifier, links on whatever the switch says, which of the similar items each filed note or task in one space should link to, three calls at a time, `ai_calls` kind `relink`. It skips a task whose reminder has not fired (the reminder loop reads filed items only, so a reminder due while it waited would be written off) and an item whose text is unchanged since it was last asked (`link_asks`, keyed by a hash of the text; `--dry-run` records nothing). One suggestion per pair: `store.pair_known` counts `link_suggestions` in any state and existing links either way, so a skipped pair is never offered again. `store.suggest` re-checks at write time -- an item deleted or no longer filed during the call is left alone, and so is a target that went; a target sent back as `relink` earlier in the same run still counts (a pre-deploy review found such pairs lost). An item with suggestions goes back to `attention` as **`relink`**, only `stage` and `wait_reason` changing (the touch trigger does move `updated_at`). Approving a `relink` item goes through `store.refile_relinked`: its own current fields plus any edits, the links listed as kept appended as `Related: [[…]]` (none when the approval lists none), the rest marked `skipped`, `classified_at` and `proposal_json` untouched so it is not a fresh example. Redo refuses it, `reclassify --attention` skips it, and the client starts both the card's sentence and the item page's form from the item, not its proposal -- a proposal-first form sends the first reading back as edits. Known: after refiling, a kept `Related:` line is part of the text the examples quote; `reconcile_spaces` can null a waiting `relink` item's space, and approving it then needs a space chosen.
...
UI checks run headless Chrome through playwright-core from the scratchpad (the Claude in Chrome extension was not connected on 2026-09-17). **A check that stubs an API response must run in its own context with `serviceWorkers: "block"`**: the app's worker answers `/api/*` GETs itself (network first, cache fallback), and a request it serves never reaches `page.route`, so the stub is silently bypassed (found on slice 33's card check, 2026-09-22). A POST passes the worker and can be stubbed in the shared context. **Buttons are drawn uppercase**, so a label is compared case-insensitively. Contrast is read out of the running app rather than from the stylesheet, and tap targets are measured on the element that receives the tap -- a checkbox's target is the `.tap-box` label wrapping it, not the 18px box.
