...
**Seven themes, all dark**, chosen in Settings and stored in `tartib-theme`; bronze
(`#0d0f12` / `#b98a44`) is the default and the one the type and spacing were tuned against. The
other six were given as *light* palettes and are built the other way round -- the palette's
primary is the ground, its accent stays the accent -- so **no theme's accent sits in its own
background's hue family**, which is the rule six scrapped palette passes produced. Nothing
follows the OS. Every value is generated by `frontend/tools/themes.py`, which enforces the
floors; `../intent/themes.md` is the record, and a hex in the palette section of `styles.css` is
never edited by hand. Beside the accent there are four tones -- `--ok`, `--info`,
