...
- ~~**AI usage on record**~~ — **done in slice 29** (2026-09-22), `slice-29-ai-usage.md`.
  One row per AI call, the subscription limit as its own kind of failure, and a costed estimate
  that reconciles to the penny. The quota readout is built but dormant: `codex exec` emits no
  `token_count` event. Count the calls and the tokens, per call and in total, and show them.
