...
import { formatDueLong, formatLongDate, formatRelative, todayLocal } from "../format";
    ...
    <div className="screen">
      ...
      <div className="dash">
        <div className="dash-main">
          <Card icon={<StarIcon />} label="Today" aside={data ? items.length : "…"}>
          ...
          <Card icon={<ClockIcon />} label="Recent" aside={<span className="muted">last {data?.recent.length ?? 10}</span>}>
            {data && data.recent.length === 0 && <p className="empty muted">Nothing captured yet.</p>}
            <ul className="rows flat">{data?.recent.map((cap) => <RecentRow key={cap.id} cap={cap} />)}</ul>
          </Card>
        </div>
        <aside className="dash-side">
          <Card icon={<AlertIcon />} label="Needs attention" aside={attention ? attention.items.length + attention.stale.length : "…"}>
            {attention && attention.items.length === 0 && attention.stale.length === 0 && <p className="empty muted">All caught up.</p>}
            <ul className="rows flat">
                  ...
                  <div className="row-main">
                    ...
                    <div className="row-body">
                      ...
                      <span className="row-meta muted">
                        ...
                        {item.proposal?.space && <> · {item.proposal.space}</>}
                        {item.proposal && <> · {Math.round(item.proposal.confidence * 100)}%</>}
                        {!item.proposal?.space && <> · no space fits</>}
                      </span>
                    </div>
                  </div>
                </li>
            ...
            {attention && attention.items.length + attention.stale.length > 3 && (
          ...
          {data?.active_space && (
            <p className="side-note muted">
              <Link to={`/search?space=${data.active_space}`}>{data.active_space}</Link> is the most recently touched space.
            </p>
          )}
        </aside>
      </div>
...
function RecentRow({ cap }: { cap: CaptureRecord }) {
  ...
  const state = cap.status === "pending" ? "filing…" : cap.answer && cap.items.length === 0 ? "answered" : first?.stage === "attention" ? "needs attention" : first?.space ?? null;
  return (
    <li className="row">
      <div className="row-main">
